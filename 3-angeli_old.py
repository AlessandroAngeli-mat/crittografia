...
This code has been rated 9.87/10 by pylint 2.16.1
(points detucted due to module name 3-angeli not conforming to snake case)
"""
...
def decrypt_key(key):
    ...
    try:
        ...
        print('Error while decrypting key: '+str(value_error))
        # let user abort
        choice = input('q to quit, anything else to try again: ')
        if choice.lower() == 'q':
            # abort
            raise HybEncError(value_error) from value_error
...
def encrypt_file():
    ...
    try:
        ...
        if choice.lower() == 'q':
            #abort
            raise EncryptionError() from value_error
...
def decrypt_file():
    ...
    ciphertext, _ = read_file('encrypted', 'Error reading encrypted file', 'encrypted.enc')
